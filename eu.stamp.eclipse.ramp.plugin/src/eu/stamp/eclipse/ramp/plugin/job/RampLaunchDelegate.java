...
public class RampLaunchDelegate extends JavaLaunchDelegate{
	...
	//Add Botsing library
	URL url = new URL(RampPluginConstants.EVOSUITE_JAR_URL);
	collection.add (FileLocator.toFileURL(url).getPath());
	/*
	url = new URL(JUNIT_JAR_URL);
	collection.add(FileLocator.toFileURL(url).getPath());
	
	url = new URL(HAMCREST_JAR_URL);
	collection.add(FileLocator.toFileURL(url).getPath());
	
	url = new URL(MOCKITO_JAR_URL);
	collection.add(FileLocator.toFileURL(url).getPath());
*/
