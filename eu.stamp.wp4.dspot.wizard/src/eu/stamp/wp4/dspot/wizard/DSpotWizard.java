...
public class DSpotWizard extends Wizard {
	...
	// [0] Dspot jar path, [1] project path, [2] number of iterations i, [3] -t test
	// class, [4] -a Method
	...
	public boolean performFinish() {
		...
		if (System.getenv("MAVEN_HOME") == null && (advParameters[4] == null || advParameters[4] == "")) { // an error
																											// message
																											// if
																											// MAVEN_HOME
																											// is not
																											// set
			Shell shell = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell();
			MessageDialog.openError(shell, "Maven Home not set",
					"Error the enviroment variable MAVEN_HOME is required, please set it in your computer or in the text in advanced options in page 2");
		} else { // if MAVEN_HOME is set
			writeTheFile(); // writing the properties file
			// MessageConsole MyConsole = createConsole("Dspot Console"); // obtaining the
			// console of the eclipse application
			...
			Job job = new DSpotEclipseJob(parameters, advParameters, verbose, clean, wConf); // execute Dspot in
																								// background
	...
	/**
	 * this is the method to write the dspot.properties, it is called by
	 * performFinish it uses the information in page 1 and it is called by
	 * performFinish
	 */
